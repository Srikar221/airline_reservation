...
struct airline             // structures are used to represent a record
{
	char passport[6];
	char name[15];
      char destination[15];
...
int main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   WELCOME TO Srikar's AIRLINE SYSTEM                   ");
		printf("\n\t\t   *******************************************************************");
		...
		printf("\n\n\t\t 3. DISPLAY RECORDS");
		printf("\n\n\t\t 4. EXIT");
		printf("\n\n\t\t feel free to contact group17@gmail.com");
		printf("\n\n\t\t Enter your choice :");
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct airline*)malloc(sizeof(struct airline)); // The malloc   in C is used to dynamically allocate a single large block of memory with the specified size.
		details();
